...
const lowestPrice = library.reduce((minPrice, category) => {
    const categoryMinPrice = category.books.reduce((minPriceInCategory, book) => {
        return Math.min(minPriceInCategory, book.price);
    }, Infinity); 

    return Math.min(minPrice, categoryMinPrice);
}, Infinity); 

console.log("Maziausia knygos kaina: ", lowestPrice);
